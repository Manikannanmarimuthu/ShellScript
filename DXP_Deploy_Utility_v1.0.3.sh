...
echo "Not Necessary to stop BATCH Process. Since scripts are running in $action_hostname2"
else
    echo "Hostname does not match any expected hostnames"
fi
}
...
echo "Not Necessary to stop PARTITIONSERVICE Process. Since scripts are running in $action_hostname2"
else
    echo "Hostname does not match any expected hostnames"
fi
}
...
echo "Not Necessary to stop WSPROCESS Process. Since scripts are running in $action_hostname2"
else
    echo "Hostname does not match any expected hostnames"
fi
...
cd /hlfapp/; tar -czf DXPApp_bk_${NOW}.tar.gz online;
sleep 2s
status_check
...
yes | cp -rf /hlfapp/Deploy/backup/${NOW}/batch/logfiles/*  /hlfapp/DXPApp/batch/logfiles/ >> "${LOG}" 2>&1;
status_check
elif [ "$current_hostname" = "$action_hostname2" ]; then
...
yes | cp -rf //hlfapp/Deploy/backup/${NOW}/partitionservice/logfiles/*  /hlfapp/DXPApp/partitionservice/logfiles/ >> "${LOG}" 2>&1;
status_check
elif [ "$current_hostname" = "$action_hostname2" ]; then
