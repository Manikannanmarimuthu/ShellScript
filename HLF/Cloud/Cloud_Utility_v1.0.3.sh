...
CLOUD_PARAM_OUTPUT_BASE="cloud_params_output.txt"
CLOUD_PARAM_OUTPUT="${environment}_${CLOUD_PARAM_OUTPUT_BASE}"

# Function to run an AWS Secrets Manager command and capture the output
run_aws_secretsmanager_command() {
    echo "-------------Secret Manger--------------" >> "$CLOUD_PARAM_OUTPUT"
    local secret_id="$1"
    local parameter_name="$2"
    command="aws secretsmanager get-secret-value --secret-id '$secret_id' --query 'SecretString' --output text | tee -a '$CLOUD_PARAM_OUTPUT'"
    echo "Running AWS Secrets Manager command:"
    echo "$command"
    eval "$command"
    # Capture the output in the specified format
    echo "$parameter_name : $(cat "$CLOUD_PARAM_OUTPUT" | tail -n 1)" >> "$CLOUD_PARAM_OUTPUT"
    echo "---------------------------" >> "$CLOUD_PARAM_OUTPUT"
}

# Function to run an AWS SSM Parameter Store command and capture the output
run_aws_ssm_parameter_command() {
    echo "-------------Parameter Store --------------" >> "$CLOUD_PARAM_OUTPUT"
    local name="$1"
    local parameter_name="$2"
    command="aws ssm get-parameter --name '$name' --query Parameter.Value --output text | tee -a '$CLOUD_PARAM_OUTPUT'"
