...
if [ "$environment" != "vit" ]; then
    ...
    echo "Skipping extraction. This script is designed for environments other than $environment."
fi
...
mv $extract_path/app.tar.gz /hlfapp/Deploy/backup/${NOW}/ >>${LOG};
sleep 2s
status_check
...
myslq_jar = "mysql-connector-j-8.0.33.jar"
online_lib_loc = "/hlfapp/DXPApp/online/mdynamics/lib/3rdparty"
auth_lib_loc = "/hlfapp/DXPApp/auth/lib/"


print_info "Enabling Soft Link for Online"
   file_path= "$online_lib_loc/$myslq_jar" "/hlfapp/DXPApp/online/mdynamics/lib/3rdparty/mysql-connector-j-8.0.33.jar"
   if [ -f "$file_path" ]; then
       echo "MYSQL-CONNECTOR-J-8.0.33.JAR exists: $file_path :: Proceeding to delete mysql-connector-j-8.0.33.jar then enable soft link"
	     rm -rf $online_lib_loc/mysql-connector-j-8.0.33.jar;
	     cd $online_lib_loc; ln -s $mysql_connector_jar;	
   else
