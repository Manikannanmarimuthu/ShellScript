...
if ![ "$environment" = "vit" ]; then
    ...
    echo "The environment variable 'env' is equal to 'some_value'."
else
    # Code to execute when the condition is false
    echo "The environment variable 'env' is not equal to 'some_value'."
fi


if
print_head "Procceding to remove the http.properites /hlfapp/DXPApp/online/logfiles/ "
yes | cp -rf "/hlfapp/Deploy/backup/${NOW}/DXPApp/inquiry/logfiles/"* "/hlfapp/DXPApp/online/mdynamics/bin/http.properties" >> "${LOG}" 2>&1
...
function inner_select() {
    ...
    case "$choice" in
        Deploy)
		 while true; do
          PS3="Select an option from the listed options above ("1 DEPLOY_HLFDXP" "2 backToParentMenu") : "  # Customize the prompt
              ...
              if confirm_action "Deploy" "HLF_DXP"; then
                       ...
                       if [ -e "$directory/$filename" ]; then
                         echo "$filename exists in $directory."
                          ...
                          echo "$filename does not exist in $directory. Kindly place valid app.tar.gz "
                          break;
                        fi
                            format_and_display_jps
						                deploy_hlfdxp
        ...
        Shutdown)
          ...
          options=("AUTH" "BATCH","DATASYNC","INQUIRY","ONLINE","WSPROCESS","PARTITIONSERVICE","SHUTDOWNALL","backToCurrentMenu","backToParentMenu")
          select choice in "${options[@]}"; do
          case "$choice" in
                "AUTH")
