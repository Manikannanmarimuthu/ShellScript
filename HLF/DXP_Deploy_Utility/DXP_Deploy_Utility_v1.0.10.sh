...
check_process_status() {
    ...
    if ps aux | grep -v grep | grep -q "$process_name"; then
        display_color "cyan" "$process_name is running now."
    else
        display_color "yellow" "$process_name is not running now."
        echo
        echo "*********************************"
...
shutdown_wsprocess(){
  if [ "$current_hostname" = "$action_hostname1" ]; then
print_head "Checking the status of the WSPROCESS service..."
...
print_head "Proceeding to remove the http.properites /hlfapp/DXPApp/online/mdynamics/bin/http.properties and restroing from backup "
if ! [ "$environment" = "vit" ]; then
