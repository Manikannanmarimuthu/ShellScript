...
if [ "$environment" = "vit" ]; then
  ...
  mysql_connector_jar="/hlfapp/lib/mysql-connector/mysql-connector-j-commercial-8.0.33"
...
if [ "$environment" = "vit" ]||[ "$environment" = "sit" ]; then
  print_info "Since scripts are running in "$environment" Enabling Soft Link"
...
print_info "Since scripts are running in "$environment" copying mysql-connector-j-8.0.33.jar from hlfapp/DXPApp/batch/lib/mysql-connector-j-8.0.33.jar "
