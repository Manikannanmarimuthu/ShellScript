...
elif [ "$ENVIRONMENT" == "vit" ]; then
    DEPLOY_DIR="/hlfapp/Deploy/vitlogfiles"
    LOG_DIRS=(
        "/hlfapp/DXPApp/auth/logfiles"
        "/hlfapp/DXPApp/datasync/logfiles"
        "/hlfapp/DXPApp/inquiry/logfiles"
...
"sit")
    ...
    zip -r "$TARGET_DIR/$current_hostname-logs.zip" "$TARGET_DIR"
    print_info "Logs zipped successfully."

    print_info "Removing original log files..."
    find "$TARGET_DIR" -type f -name "*${CONVERTED_DATE}*" -exec rm -f {} +
    if [ "$DATE" == "$(date "+%Y-%m-%d")" ]; then
        find "$TARGET_DIR" -type f -name "mvi-services.log" -exec rm -f {} +
    fi
...
"vit")
    ...
    print_info "Zipping logs in $TARGET_DIR..."
    zip -r "$TARGET_DIR/$current_hostname-$DATE-logs.zip" "$TARGET_DIR"
    print_info "Logs zipped successfully."
