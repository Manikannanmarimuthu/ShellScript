...
check_process_status() {
    ...
    if ps aux | grep -v grep | grep -q "$process_name"; then
        ...
        display_color "cyan" "$process_name is not running now."
        echo
        echo "*********************************"
          ...
          case "$choice" in
              ...
              if confirm_action "Deploy" "HLF_DXP"; then
                       ...
                       if [ -e "$directory/$filename" ]; then
                          print_head "$filename exists in $directory."
                      else
                          print_head "$filename does not exist in $directory Kindly place valid app.tar.gz"
                          break;
                        fi
                            format_and_display_jps
						                deploy_hlfdxp
