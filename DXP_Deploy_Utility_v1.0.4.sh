...
display_color "cyan" "Not Necessary to stop BATCH Process. Since scripts are running in $action_hostname2"
else
    echo "Hostname does not match any expected hostnames"
fi
}
...
display_color "cyan" "Not Necessary to stop PARTITIONSERVICE Process. Since scripts are running in $action_hostname2"
else
    echo "Hostname does not match any expected hostnames"
fi
}
...
display_color "cyan"  "Not Necessary to stop WSPROCESS Process. Since scripts are running in $action_hostname2"
else
    echo "Hostname does not match any expected hostnames"
fi
}
...
if [ -n "$(find . -maxdepth 1 -type f \( -name "*.log" -o -name "*_c.log" -o -name "*_default.log" \))" ]; then
 ...
 echo "Not required to copy the log files for BATCH, Since scripts are running in $action_hostname2"
else
    echo "Hostname does not match any expected hostnames"
fi
...
status_check
   ...
   echo "Not required to copy the log files for PARTITIONSERVICE, Since scripts are running in $action_hostname2"
else
    echo "Hostname does not match any expected hostnames"
fi
            ...
            select sub_option in AUTH BATCH DATASYNC INQUIRY ONLINE WSPROCESS PARTITIONSERVICE SHUTDOWNALL backToCurrentMenu backToParentMenu; do
                case "$sub_option" in
					...
					ONLINE)
                        ...
                        ;;		
          WSPROCESS)
					 print_process_status "Input received from user to SHUTDOWN HLF-DXP WSPROCESS Service"
					  if [ "$current_hostname" = "$action_hostname1" ]; then
					   shutdown_wsprocess
                      elif [ "$current_hostname" = "$action_hostname2" ]; then
            ...
            select sub_option in AUTH BATCH DATASYNC INQUIRY ONLINE WSPROCESS PARTITIONSERVICE STARTALL backToCurrentMenu backToParentMenu; do
                case "$sub_option" in
					...
					ONLINE)
                        ...
                        ;;
          WSPROCESS)
					 print_process_status "Input received from user to start HLF-DXP WSPROCESS Serivce"
					  if [ "$current_hostname" = "$action_hostname1" ]; then
					   start_wsprocess
                      elif [ "$current_hostname" = "$action_hostname2" ]; then
                          echo "In HLF-DXP WSPROCESS does not run in $action_hostname2 will run only $action_hostname1"
            ...
            select sub_option in AUTH BATCH DATASYNC INQUIRY ONLINE WSPROCESS PARTITIONSERVICE RESTARTALL backToCurrentMenu backToParentMenu; do
                case "$sub_option" in
					...
					ONLINE)
                        ...
                        ;;
          WSPROCESS)
					print_process_status "Input received from user to restart HLF-DXP WSPROCESS Service"
					  if [ "$current_hostname" = "$action_hostname1" ]; then
					    restart_wsprocess
                      elif [ "$current_hostname" = "$action_hostname2" ]; then
                          echo "In HLF-DXP WSPROCESS does not run in $action_hostname2 will run only $action_hostname1"
