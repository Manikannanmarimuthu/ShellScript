...
if [ "$environment" = "vit" ]; then
  ...
  insert_command_after_word "# NewRelic APM for Auth" "export JAVA_OPTS="$JAVA_OPTS -javaagent:/opt/newrelic-dxp-auth/newrelic.jar -Dnewrelic.environment=uat"" "/hlfapp/DXPApp/auth/bin/proj-hlfdxp-auth.sh" "no"  

  print_info "Adding NewRelic APM for DataSync"
  insert_command_after_word "CLASSPATH=$APP_HOME" "# NewRelic APM for Batch" "/hlfapp/DXPApp/datasync/bin/proj-hlfdxp-datasync" "yes"
  insert_command_after_word "# NewRelic APM for Batch" "export JAVA_OPTS="$JAVA_OPTS -javaagent:/opt/newrelic-dxp-datasync/newrelic.jar -Dnewrelic.environment=uat"" "/hlfapp/DXPApp/datasync/bin/proj-hlfdxp-datasync" "no"  

  print_info "Adding NewRelic APM for Inquiry"
  insert_command_after_word "CLASSPATH=$APP_HOME" "# NewRelic APM for Inquiry" "/hlfapp/DXPApp/inquiry/bin/proj-hlfdxp-inquiry" "yes"
...
if [ "$current_hostname" = "$action_hostname1" ]; then
  ...
  insert_command_after_word "CLASSPATH=$APP_HOME" "# NewRelic APM for Batch" "/hlfapp/DXPApp/batch/bin/proj-hlfdxp-batch" "yes"
  insert_command_after_word "# NewRelic APM for Batch" "export JAVA_OPTS="$JAVA_OPTS -javaagent:/opt/newrelic-dxp-batch/newrelic.jar -Dnewrelic.environment=uat"" "/hlfapp/DXPApp/batch/bin/proj-hlfdxp-batch" "no" 
fi

else
  display_color "cyan" "Since its: $environment Its not required to enable relic"
fi
}
